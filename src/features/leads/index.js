...
function Leads() {
    ...
    useEffect(() => { setActiveBtn(null); }, [selectedDevice?.id]);
                        ...
                        {filteredDevices.map((device) => {
                            return (
                                <div key={device.id} className='absolute' style={{ top: device.position.top, left: device.position.left }}>
                                    <img src={device.img} alt={device.name} className="w-16 h-16 rounded-lg bg-[#69696B] border-2 border-white" />
                                </div>
                            )
                        })}
                    </div>
                                        ...
                                        <div className="grid gap-2 mb-4">
                                            ...
                                            {selectedDeviceCheck === 'Airconditioner' ?
                                                <div className="flex gap-2">
                                                    <button onClick={() => setActive(!active)} className={`${active ? 'bg-[#166B19E3]' : 'bg-gray-400'} p-2 rounded-md flex justify-center items-center hover:bg-green-900`}>
    ...
    ${selectedDevice.mode === 'fan' ? 'border-[2px] border-[#4472C4] bg-[#b2ccfa]' : ''}`}
                                                        >
                                                            <svg fill={`${activeBtn === 'fan' ? '#4472C4' : ''}`} className="w-6 h-6" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    ${selectedDevice.mode === 'cool' ? 'border-[2px] border-[#4472C4] bg-[#b2ccfa]' : ''}`}
                                                        >
                                                            <svg viewBox="0 0 45 45" className="w-6 h-6" fill={`${activeBtn === 'cool' ? '#4472C4' : ''}`} xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        {selectedDeviceCheck === 'Airconditioner' ?
                                            <div className="grid pl-3 gap-3">
                                                {selectedDevice.mode === 'cool' ? <>
                                                    ...
                                                    </div></> : <div className="w-full grid gap-2 mb-3"></div>}
            ...
            </div >
            {
                isScheduleOpen && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center">
                        {/* backdrop */}
                        <div className="absolute inset-0 bg-black/40" onClick={() => {setIsScheduleOpen(false); setSelectedDays(null);}} />
                        ...
                        <div className="relative z-10 w-full max-w-md rounded-xl bg-base-100 p-4 shadow-xl">
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <button className="btn btn-ghost btn-sm" onClick={() => {setIsScheduleOpen(false); setSelectedDays(null);}}>✕</button>
                            </div>
                ...
                ${active ? 'bg-[#4472C4] text-white' : 'bg-gray-300 hover:bg-gray-500'}`}>
                            ...
                            <div className="mt-4 flex justify-end gap-2">
                                <button
                                    ...
                                    onClick={() => {setIsScheduleOpen(false); setSelectedDays(null);}}
                                >
                                    Cancel
                                </button>
                                <button
                            ...
                            {selectedDevice && schedules[selectedDevice.id] && (
                                <p className="mt-3 text-xs text-gray-500">
                                    Current for <span className="font-medium">{selectedDevice.name || `Device #${selectedDevice.id}`}</span>:
                                    {' '}{schedules[selectedDevice.id].timeStart} - {schedules[selectedDevice.id].timeEnd} / {schedules[selectedDevice.id].days.join(', ') || '—'}
                                </p>
                            )}
                        </div>
                    </div>
