...
import { fetchGroups, fetchSingleDevice, fetchDeviceData } from "../common/groupSlice";
...
function Leads() {
    ...
    const handleDeviceClick = (device, detail) => {
        setSelectedDevice((device !== selectedDevice ? device : null) && (detail !== selectedDevice2 ? detail : null));
    ...
    const bgimage = () => {
        if (selectedBuilding === "Zone A" && selectedFloor === '1') {
            return "../image/buildingA1.png";
        } else if (selectedBuilding === "Zone A" && selectedFloor === '2') {
            return "../image/buildingA2.png";
        } else if (selectedBuilding === "Zone A" && selectedFloor === '3') {
            return "../image/buildingA3.png";
        } else if (selectedBuilding === "Zone B" && selectedFloor === '1') {
            return "../image/buildingB1.png";
        } else if (selectedBuilding === "Zone B" && selectedFloor === '2') {
        ...
        <div className="flex h-full">
            <div className="bg-base-100 shadow-xl border-r p-2 w-[25%] h-auto">
                <div className="grid gap-2">
                                ...
                                <div className="p-2 border bg-[#F4F4F4]">
                                    ...
                                    <ul>
                                        <li
                                            ...
                                            onClick={() => { handleDeviceSelection('Airconditioner'); setChanged('ac') }}
                                        >
                                        ...
                                        <li
                                            ...
                                            onClick={() => { handleDeviceSelection('fanfilter'); setChanged('ffu') }}
                                        >
                                            <label htmlFor="" className="flex gap-1">
                    ...
                    <div className="overflow-x-auto h-[75vh] grid ">
                        {selectedMode === 'group' ?
                            <div className="w-full">
                                <div className="grid gap-2">
                                    {(groups || []).map((device) => {
                                        if (device.type !== Changed) return null;

                                        const singleData = single[device.members[0]];
                                        const speedPercentage = (singleData?.fan_speed / 100) * 100;
                                            ...
                                            <div key={device.id} className={`flex justify-between w-full gap-1 hover:bg-gray-100 cursor-pointer ${selectedDevice?.id === device.id ? 'bg-[#EBEEFD]' : ''}`} onClick={() => handleDeviceClick(device, singleData)}>
                                                ...
                                                {selectedDeviceCheck === 'Airconditioner' ?
                                                    ...
                                                    <div className="grid gap-1 gap-y-2 w-full items-start">
                                                        <p className="text-sm flex gap-1"><img src="../icon/computer-fan-svgrepo-com.svg" className="w-5 h-5" alt="" />{speedPercentage} <span className="text-sm">speed</span></p>
                            ...
                            <div className="">
                                {Object.keys(single || {}).map((deviceId, idx) => {
                                    ...
                                    if (filteredDevice) {
                                        ...
                                        return (
                                            <div key={device?.id} className='my-1' onClick={() => handleDeviceClick(filteredDevice, device)}>
                                                <div className={`flex gap-2 items-center w-full hover:bg-gray-100 cursor-pointer ${selectedDevice?.device_name === deviceId ? 'bg-[#EBEEFD]' : ''}`}>
                                                    <img src={icon} alt="Device" className="w-18 h-18" />
                                                    <div className='flex justify-start w-full '>
                                                        <div className="flex flex-col gap-1 w-full">
                                                            <div className="flex justify-between w-full mb-1">
                                                                <p className="font-semibold">{deviceId}</p>
                                                                {device?.power === 'on' ?
                                                            ...
                                                            {selectedDeviceCheck === 'Airconditioner' ?
                                                                ...
                                                                <div className="grid grid-cols-2 gap-1 gap-y-2 items-center">
                                                                    <p className="text-sm flex gap-1"><img src="../icon/computer-fan-svgrepo-com.svg" className="w-5 h-5" alt="" />{device?.fan_speed} <span className="text-sm">speed</span></p>
                                                                    <div className="w-full h-2 bg-gray-300 rounded-full">
                                                                        <div className="h-full bg-[#0090CD] rounded-full" style={{ width: `${speedPercentage}%` }}></div>
                                                                    </div>
                    ...
                    {filteredDevices.map((device) => {
                        return (
                            <div key={device.id} className='absolute' style={{ top: device.position.top, left: device.position.left }}>
                                <img src={icon} alt={device.name} className="w-16 h-16 rounded-lg bg-[#69696B] border-2 border-white" />
                            </div>
                        )
                    })}
                </div>
                                    ...
                                    <div className="grid gap-2 mb-4">
                                        ...
                                        {selectedDeviceCheck === 'Airconditioner' ? (
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                <input
                                                    type="text"
                                                    value={selectedDevice2?.fan_speed}
                                                    readOnly
